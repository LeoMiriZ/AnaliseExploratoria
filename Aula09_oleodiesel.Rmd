...
dados=read_excel("Aula 09 - Óleo Diesel.xlsx", sheet = "Diesel")
oleo = ts(dados$Consumo, start = c(2011,1), frequency = 12)
oleo

plot(oleo, main = "Buscas em % pelo termo óleo")
```
...
decomp <- decompose(oleo, type = "multiplicative")
...
########### MODELAGEM ###############
library(forecast)
model <- auto.arima(oleo)

# summary(model)

f <- forecast(model, h = 12)
library(ggplot2)
autoplot(f, xlim=c(2015,2017))
